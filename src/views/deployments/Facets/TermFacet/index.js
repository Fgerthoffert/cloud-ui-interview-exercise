...
import ExpandButton from "../../../../components/facets/ExpandButton";
    ...
    return (
      <React.Fragment>
        <EuiPageContent style={{ padding: "10px" }}>
          ...
          <EuiFacetGroup style={{ maxWidth: 200 }}>
            {facetsData.map(term => (
              <Term
                ...
                facet={facet}
                selected={selectedValues.indexOf(term.value) !== -1}
                {...term}
              />
            ))}
          </EuiFacetGroup>
          <ExpandButton
            collapsed={collapsed}
            length={facet.values.length}
            onClick={this.collapseFacet}
          />
        </EuiPageContent>
        <EuiSpacer size="m" />
      </React.Fragment>
